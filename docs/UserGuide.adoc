...
* **`checkin`**`n/ Smith rn/101` : checks in `Smith` to room `101` in the hotel.
* **`checkout`**`rn/101` : checks out guest from room `101` and removes guest from guest list.
...
* Words in `UPPER_CASE` are the parameters to be supplied by the user e.g. in `checkin n/NAME rn/ROOM_NUMBER`, `NAME` is a parameter which can be used as `add n/Smith`.
...
Format: `checkout n/NAME rn/ROOM_NUMBER`

Examples:

* `checkout n/Smith rn/101`
