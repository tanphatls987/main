...
1.  The command from user will be parsed into a list of pattern contained in the search command.
2.  Then the command when execute will filter guest list base on stored pattern.

Below is the diagram to show the execution of the command.
...
* `RemoveChargeServiceCommand` -- Removes a charged service from the bill of a guest.
...
==== RemoveChargeService Command Implementation
===== Implementation
===== Design Considerations
