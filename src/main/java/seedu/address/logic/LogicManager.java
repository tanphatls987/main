...
import seedu.address.logic.parser.MorpheusParser;
...
public class LogicManager implements Logic {
    ...
    private final MorpheusParser morpheusParser;

    public LogicManager(Model model, Storage storage) {
        this.model = model;
        this.storage = storage;
        morpheusParser = new MorpheusParser();
    }
    ...
    public CommandResult execute(String commandText) throws CommandException, ParseException {
        ...
        Command command = morpheusParser.parseCommand(commandText);
        commandResult = command.execute(model);

        try {
            storage.saveAddressBook(model.getAddressBook());
            storage.saveHotel(model.getHotel());
            storage.saveBookKeeper(model.getBookKeeper());
