package seedu.address.storage;

import java.io.IOException;
import java.nio.file.Path;
import java.util.Optional;

import seedu.address.commons.exceptions.DataConversionException;
import seedu.address.model.ReadOnlyBookKeeper;

/**
 * Represents a storage for {@link seedu.address.model.AddressBook}.
 */
public interface BookKeeperStorage {

    /**
     * Returns the file path of the data file.
     */
    Path getBookKeeperFilePath();

    /**
     * Returns BookKeeper data as a {@link ReadOnlyBookKeeper}.
     *   Returns {@code Optional.empty()} if storage file is not found.
     * @throws DataConversionException if the data in storage is not in the expected format.
     * @throws IOException if there was any problem when reading from the storage.
     */
    Optional<ReadOnlyBookKeeper> readBookKeeper() throws DataConversionException, IOException;

    /**
     * @see #getBookKeeperFilePath()
     */
    Optional<ReadOnlyBookKeeper> readBookKeeper(Path filePath) throws DataConversionException, IOException;

    /**
     * Saves the given {@link ReadOnlyBookKeeper} to the storage.
     * @param bookKeeper cannot be null.
     * @throws IOException if there was any problem writing to the file.
     */
    void saveBookKeeper(ReadOnlyBookKeeper bookKeeper) throws IOException;

    /**
     * @see #saveBookKeeper(ReadOnlyBookKeeper)
     */
    void saveBookKeeper(ReadOnlyBookKeeper bookKeeper, Path filePath) throws IOException;

}

